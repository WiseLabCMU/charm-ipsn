...
\label{alg:algorithm-label}
 packets=receive\_data();\;
 \For{packet in packets}{
 packet=$e^{j2\pi \Delta ft}$packet \tcp*{Frequency Offset}
 packet=$e^{j2\pi f\Delta t}$packet \tcp*{Timing Offset Correction}
 h(packet)$=\frac{packet}{reference}$ \tcp*{Channel Estimation}
 }
 combined\_packet$=$zeros(size(packet))\;
 \For{packet in packets}{
 combined\_packet+=h(packet)$^*$packet;
 }
 return combined\_packet\;
\end{algorithm}
