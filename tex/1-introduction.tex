...
%This paper presents Charm, a system that enhances \revathy{the network coverage of LPWAN clients in addition to increasing its battery life}\cmt{the battery-life and coverage of LP-WAN clients}{reword this... clients dont have coverage, the network does} in large urban deployments. 

\akshay{This paper presents Charm, a system that enhances the coverage of LP-WAN base stations in large urban deployments, in turn, improving the battery-life and throughput of the LP-WAN clients.} Charm exploits the observation that while signals from certain clients may attenuate significantly, they are still likely to be received by multiple gateways in a dense network. Charm develops a hardware and software design at the gateways that identifies and transports weak received signals to the cloud. We then develop a joint decoding system at the cloud that coherently combines weak signals received across multiple city gateways to decode the underlying data. As a result, Charm both expands the decoding range of the LP-WAN network and improves battery-life for nodes already in range -- allowing clients across the network to spend less energy per transmitted bit. We implement Charm on the LoRaWAN platform~\cite{LoRaWanAlliance2015}, a popular and widely available LP-WAN technology. \akshay{Charm is implemented in a first-of-its-kind low-power wide-area pilot deployment for coherent diversity combining. We first use a campus-scale deployment using LoRa gateways, which is used to evaluate coverage, penetration inside buildings and battery life. We also perform micro-benchmarks using Charm gateways on a smaller testbed and use the traces to simulate the effects of Charm on a variety of configurations. Both of the above deployments serve a large neighborhood of a major U.S. city spanning multiple buildings and open spaces between them and demonstrate increased coverage and battery-life across clients in the network.}

\cmt{Charm is implemented in a first-of-its-kind low-power wide-area pilot deployment for coherent diversity combining. This deployment serves a large neighborhood of a major U.S. city and demonstrates increased coverage and battery-life across clients in the network.}{clarify using 2 testbeds}
\revathy{ This has been verified using two test-beds, one being a major deployment covering approximately 10 square kilometer in a major US city, and a smaller one for micro benchmarking. The testbeds result demonstrate a clear increase in coverage and battery-life across clients in the network.  }
...
system for low-power WANs offers radically new challenges. \akshay{Although the density of LP-WAN gateways is expected to rise, the signals of many of the deployed client LP-WAN devices will be often too weak to be ordinarily detected -- sometimes even 30 dB below the noise floor.}
\cmt{At the gateways, LP-WAN signals are often too weak to be ordinarily detected -- more
than 30 dB below the noise floor.}{signals being arbitrarily weak doesn't say
as much. The 30 dB provides no insight?} Yet, simply shipping all received data to the
...
to identify which of them contain packets from the same client is \akshay{another} challenge.
We provide an overview of our approach to address each of these challenges
below. 
\cmt{}{Clarify novelty of our approach... no need to time-sync, noise resilience, using data part of packet, mid-way hardware between dedicated radio and SDR}


\cmt{}{Add soemthing about time sync, cost and SDR performance requirements}
...
\akshay{ However, since the LoRaWAN basestations will not be time synchronized, wireless signals from the cheap LP-WAN clients will experience a Carrier Frequency Offset (CFO) induced due to hardware imperfections in their clocks. Moreover, this shift in frequency is continuous and the induced phase shift accumulates over time. This means that the same packet received at multiple receivers will not coherently combine due to these erratic phase shifts. A n\"aive approach of estimating frequency offset and correcting for it will not work due to high noise-to-signal power ratio. We instead employ an approach where it is sufficient to estimate the effect of the CFO modulo the minimum possible frequency separation between two encoded data chirps which becomes feasible, even under noise, due to the use of large number of data and preamble symbols in the packet.}
\cmt{}{Mention time sync requirements and cost somewhere?}
% Gateways receive transmissions from distant clients which are far below the
% noise floor. These transmissions are not only difficult to detect but also
% cannot be decoded independently. {\color{blue} (Add actual numbers?)} Using
% an approach similar to {\color{red}~\cite{} (Cloud RAN?)} would require
% continuously streaming all the raw received signals to a cloud-based decoder
% for both detection and decoding. Such an approach places a heavy cost on the
% internet bandwidth between the gateway and the cloud, which is particularly
% infeasible for UGWs.% This means the devices will have to continuously
% stream all their received raw signal data through a backbone for packet
% detection and decoding the data. This imposes a huge cost on the bandwidth
% of the backbone and also is also infeasible at low latency. To make matters
% worse, the received signals are challenging to synchronize, given the timing
% and frequency offsets between the base stations themselves.
...
\cmt{We evaluate Charm on a large university campus with four rooftop gateways
and eight indoor user-deployed gateways built using our custom hardware
platform. We study Charm's coverage and performance indoors and outdoors
across a 10 square kilometer area surrounding the campus.}{mention using 2 testbeds}
\revathy{In addition, we perform micro-benchmarks using Charm gateways using a smaller testbed covering an area of 0.6 square kilometer.} 

\akshay{We, first, evaluate Charm on a large university campus with four rooftop LoRaWAN gateways covering approximately 10 km$^2$ for coverage, penetration inside buildings and battery life. Next, we perform micro-benchmarks using eight indoor user-deployed gateways built using our custom hardware platform on a smaller testbed covering an area of 0.6 km$^2$ . Finally, we use the above traces and micro-benchmarks to simulate the effects of Charm on a variety of configurations. Both of the above deployments serve a large neighborhood of a major U.S. city spanning multiple buildings and open spaces between them.} Our results reveal the following:
